package com.cts.handson.resource;
...
import com.cts.handson.exception.UserNotFoundException;
import com.cts.handson.repository.UserRepository;
...
public class UserResource {
	...
	public Optional<Skills> getSkillsById(@PathVariable Long id) {
		...
		if(!Skills.isPresent())
        {
            throw new UserNotFoundException("The Skills with id - "+id+ " does not exists");
        }
			return Skills; 
	...
	public List<Skills> getSkillsByDescription(@PathVariable String description) {
		List<Skills> Skills = SkillsRepository.findByDescription(description);
		if(Skills.size()==0)
        {
            throw new UserNotFoundException("The Skills with id - "+description+ " does not exists");
        }
			return Skills; 
	...
	public List<Skills> getSkillsByLevel(@PathVariable String level) {
		...
		if(Skills.size()==0)
        {
            throw new UserNotFoundException("The Skills with id - "+level+ " does not exists");
        }
			return Skills; 
